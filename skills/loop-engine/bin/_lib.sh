...
loop_codex_verdict() {
  ...
  if grep -aiqE '^VERDICT:[[:space:]]*BLOCK' "$f"; then loop_warn "裁决: BLOCK (显式 VERDICT)"; return 2; fi
  if grep -aiqE '^VERDICT:[[:space:]]*PASS'  "$f"; then loop_log  "裁决: PASS (显式 VERDICT)"; return 0; fi
