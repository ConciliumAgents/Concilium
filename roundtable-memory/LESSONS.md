...
- 主持人派活/选验证席须基于各模型的**实战画像**（非仅 roster 静态 strength）：claude(Opus) 揽 exec 会超时空转→退出执行、转任指挥+验证；hermes(deepseek) 复审偏宽松（"修复成立"即放行）、kimi(K2.7) 深挖未覆盖边界更狠→综合以 kimi 深度为主、hermes 交叉验证；异质血统利于复审。（源：圆桌提速 v4 三轮评审）
